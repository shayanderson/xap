...
> The `array` option is helpful when using objects for connection in configuration settings but array(s) return types are desired. This option is used with `SELECT` commands only.
