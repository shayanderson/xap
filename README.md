...
```
Now the `$r` variable is an array with arrays instead of an array with objects (helpful when using objects for connection in configuration settings but array(s) return types are desired).
