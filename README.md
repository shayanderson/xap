...
##### Switch Decorators
*Switch* decorators are used for commands like `add`, `count`, `del` and `mod` when the command returns a `boolean` or `integer` value and use the logic `x?:y`, for example:
```php
